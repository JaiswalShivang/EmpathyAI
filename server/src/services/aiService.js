...
const { getDoctorRecommendations, formatDoctorForResponse } = require('./doctorRecommendationService');
...
async function answerQuestion(userId, question) {
    ...
    console.log('Query classification:', {
        ...
        hasMeditationKeywords: ['meditation', 'meditate', 'mindfulness', 'relaxation', 'breathing', 'yoga']
            .some(keyword => question.toLowerCase().includes(keyword))
    });
    ...
    if (isDoctorQuery) {
        ...
        if (doctorRecommendations.doctors && doctorRecommendations.doctors.length > 0) {
            doctorRecommendations.doctors.forEach((doctor, index) => {
                responseText += `**${doctor.name}**\n`;
                responseText += `🏥 ${doctor.specialization}\n`;
                responseText += `📅 ${doctor.experience} years experience\n`;
                responseText += `🏥 ${doctor.hospital}\n`;
                responseText += `💰 ₹${doctor.consultationFee}\n`;
                responseText += `⭐ ${doctor.rating}/5\n\n`;
            });

            responseText += "💬 Would you like me to help you book an appointment with any of these doctors?";
        } else {
            responseText += "I'm currently unable to access our doctor database. Please try again later or contact our support team for assistance.";
        }
...
- If the user expresses distress, anxiety, sadness, or suicidal thoughts, respond with compassion and suggest professional help or helpline resources.
...
DOCTOR RECOMMENDATION CAPABILITIES:
- When users ask about finding doctors, recommend specialists, or need healthcare providers, ALWAYS provide doctor recommendations.
- Examples of when to provide doctor recommendations:
  - "I need a cardiologist"
  - "Find me a psychiatrist"
  - "Recommend a good doctor for..."
  - "Who can help with..."
  - "Looking for a specialist in..."
  - "Need a doctor who speaks..."
  - "Find doctors in my area"
  - "Best doctors for..."
- When providing doctor recommendations, format each doctor as:
  **Doctor Name**
  🏥 [specialties]
  ...
  💰 ₹[amount]
  ⭐ [rating]/5
- Provide only the top 2 doctor recommendations maximum.
- Always include contact/booking information when possible.
- Be helpful and informative about doctor selection.
...
// Function to detect if a query is about doctor recommendations
function isDoctorRecommendationQuery(question) {
    const lowerQuestion = question.toLowerCase();

    // First, check if this is clearly a meditation query - if so, don't treat as doctor query
    const meditationKeywords = ['meditation', 'meditate', 'mindfulness', 'relaxation', 'breathing', 'yoga'];
    if (meditationKeywords.some(keyword => lowerQuestion.includes(keyword))) {
        return false;
    }

    // Check for specific doctor/specialist mentions
    const doctorSpecificKeywords = [
        'cardiologist', 'psychiatrist', 'dermatologist', 'neurologist', 'orthopedic',
        'gynecologist', 'pediatrician', 'dentist', 'surgeon', 'urologist', 'oncologist'
    ];

    // Check for doctor role mentions
    const doctorRoleKeywords = [
        'doctor', 'physician', 'specialist', 'therapist', 'counselor', 'psychologist'
    ];

    // Check for healthcare action keywords
    const healthcareActionKeywords = [
        'recommend', 'find', 'suggest', 'looking for', 'best doctor', 'good doctor',
        'experienced doctor', 'consultation', 'appointment', 'book', 'see a doctor'
    ];

    // Check for healthcare context keywords
    const healthcareContextKeywords = [
        'medical help', 'healthcare provider', 'medical professional', 'clinician',
        'diagnosis', 'treatment', 'prescription', 'surgery', 'hospital'
    ];

    // Must have at least one doctor role AND one healthcare action/context keyword
    const hasDoctorRole = doctorRoleKeywords.some(keyword => lowerQuestion.includes(keyword)) ||
                         doctorSpecificKeywords.some(keyword => lowerQuestion.includes(keyword));
    const hasHealthcareContext = healthcareActionKeywords.some(keyword => lowerQuestion.includes(keyword)) ||
                                healthcareContextKeywords.some(keyword => lowerQuestion.includes(keyword));

    // Only return true if it has both doctor role AND healthcare context
    // This prevents false positives like "I need meditation help" from triggering doctor recommendations
    return hasDoctorRole && hasHealthcareContext;
}
